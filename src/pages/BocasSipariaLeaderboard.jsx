...
function LightningBadge() {
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-violet-100 px-2.5 py-1 text-[11px] font-black uppercase tracking-wide text-violet-800">
...
function PodiumCard({ place, player }) {
  const styles = {
    1: {
      label: "1st Place",
      medal: "🥇",
      container: "border-yellow-400 bg-yellow-100",
    ...
    >
      ...
      {player ? (
        <>
          ...
          {player.durationMs <= LIGHTNING_LIMIT_MS && (
            <div className="mt-4">
              <LightningBadge />
            </div>
          )}
...
function LeaderRow({ player, rank }) {
  return (
    <article className="grid gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center">
      <div className="flex h-11 min-w-16 items-center justify-center rounded-full bg-blue-600 px-3 text-sm font-black text-white shadow-sm">
        {ordinal(rank)}
      </div>
...
export default function BocasSipariaLeaderboard() {
  ...
  useEffect(() => {
    document.title =
      "Bocas Lit Fest Siparia Leaderboard | CountMeInTT";
    ...
    meta.content =
      "View the live CountMeInTT Bocas Lit Fest Siparia 2026 multiplication challenge leaderboard.";
  }, []);
    ...
    async ({ silent = false } = {}) => {
      ...
      try {
        ...
        try {
          ...
          throw new Error(
            "The Siparia leaderboard service did not return public data."
          );
        }
        ...
        validRows.forEach((player) => {
          const playerKey = normalize(player.name);
          const existing = bestByPlayer.get(playerKey);

          if (
            !existing ||
            compareResults(player, existing) < 0
          ) {
            bestByPlayer.set(playerKey, player);
          }
        });
        ...
        const qualifiedPlayers = uniquePlayers
          .filter(
            (player) =>
              player.durationMs <= LEADERBOARD_LIMIT_MS
          )
          .sort(compareResults);

        setTotalFinishers(uniquePlayers.length);
        setLeaders(qualifiedPlayers);
        setLastUpdated(new Date());
      } catch (error) {
        console.error(
          "Siparia leaderboard error:",
          error
        );
          ...
          <div className="mx-auto flex max-w-6xl flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
            <div className="grid min-w-0 gap-5 md:grid-cols-[330px_minmax(0,1fr)] md:items-center">
              ...
              <div className="min-w-0 md:pl-2">
                ...
                <p className="mt-3 text-base font-semibold text-gray-700 sm:text-lg">
                  5×5 Multiplication Leaderboard
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link
                ...
                className="rounded-xl bg-violet-600 px-5 py-3 text-sm font-black text-white shadow transition hover:bg-violet-700"
              >
                Hall of Fame
              </Link>
            </div>
          ...
          <div className="mx-auto max-w-6xl">
            <div className="flex flex-col gap-4 rounded-2xl border border-yellow-300 bg-white p-5 shadow-lg sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm font-semibold text-gray-500">
                  Best time per player · 25.00 seconds or faster
                </p>

                <p className="mt-2 text-xs font-bold uppercase tracking-wide text-violet-700">
                  ⚡ Lightning: 20.00 seconds or faster
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4">
                <div className="text-sm font-semibold text-gray-500">
                  <p>
                    {leaders.length} qualified · {totalFinishers}{" "}
                    {totalFinishers === 1
                      ? "finisher"
                      : "finishers"}
                  </p>
            ...
            {loadError && leaders.length > 0 && (
              <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm font-semibold text-amber-800">
                The latest refresh failed. The most recently loaded
                rankings remain on screen.
              </div>
            )}
          </div>
        </section>
          ...
          <div className="mx-auto max-w-6xl rounded-2xl border border-yellow-300 bg-yellow-50/95 p-5 shadow-lg sm:p-7">
            {isLoading ? (
              ...
              <div className="py-14 text-center">
                ...
                <h2 className="mt-5 text-2xl font-black">
                  {totalFinishers > 0
                    ? "The Speed Board is still waiting!"
                  ...
                  {totalFinishers > 0
                    ? `${totalFinishers} ${
                        totalFinishers === 1
                          ? "player has"
                          : "players have"
                      } reached the Hall of Fame. The first 25-second qualifier could be you.`
                    : "Be the first player to complete the Bocas Siparia challenge."}
                </p>
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-3">
                  {[1, 2, 3].map((place) => (
                    <PodiumCard
                      key={place}
                      place={place}
                      player={podium[place - 1] || null}
                    />
                  ))}
                </div>

                {fourthToTenth.length > 0 && (
                  <div className="mt-10">
                    ...
                    <div className="grid gap-3">
                      {fourthToTenth.map((player, index) => (
                        <LeaderRow
                          key={`${normalize(player.name)}-${player.time}`}
                          player={player}
                          rank={index + 4}
                        />
                      ))}
