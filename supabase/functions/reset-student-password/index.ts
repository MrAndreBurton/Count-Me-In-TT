...
Deno.serve(async (request) => {
  ...
  if (
    ...
    return jsonResponse(
      {
        error:
          "The password-reset service is not configured correctly.",
      },
      500
    );
  }
  ...
  try {
    ...
    if (
      ...
      return jsonResponse(
        {
          error:
            "Only an active parent account can reset a student password.",
        },
        403
      );
    }
    ...
    if (
      ...
      return jsonResponse(
        {
          error:
            "This student does not have an active login account.",
        },
        404
      );
    }
    ...
    if (
      ...
      throw (
        passwordError ||
        new Error(
          "The student password could not be updated."
        )
      );
    }

    const {
      error: updateLoginError,
    } = await adminClient
      .from("student_login_accounts")
      .update({
        login_status: "active",
    ...
    console.error(
      "reset-student-password raw error:",
      error
    );
